...
df[["acc_x" , "label"]].reset_index().boxplot(by = "label")
...
label = "bench"
for col in outlier_columns:
    df = mark_outliers_iqr(df[df["label"]==label], col) 
    plot_binary_outliers(df, col, col + "_outlier", reset_index=True)

for col in outlier_columns:
    df = mark_outliers_chauvenet(df[df["label"]==label], col) 
    plot_binary_outliers(df, col, col + "_outlier", reset_index=True)
...
outliers_removed_df.to_pickle("../../data/interim/02_data_outliers_removed_chauvenet.pickle")